...
# TODO future proof these methods against different numbers of teams and changing team names. Provide for user dir input
...
def event_process(year: str):
    """Runs bevent on files for each team in the league in a given year"""
    path = os.path.join(BASE_DATA_PATH, year)
    for team in TEAMSAL:
...
def game_process(year: str):
    """Processes bgame data, not currently used but might be necessary down the line"""
    path = os.path.join(BASE_DATA_PATH, year)
    for team in TEAMSAL:
...
def merge_files(year: str):
    ...
    for team in TEAMSAL:
        df = pd.read_csv(os.path.join(BASE_DATA_PATH, year, year + team + "INT.csv"), header=None)
        df_csv_append_int = df_csv_append_int.append(df, ignore_index=True)
        df = pd.read_csv(os.path.join(BASE_DATA_PATH, year, year + team + "STR.csv"), header=None)
...
def merge_games(year: str):
    ...
    for team in TEAMSAL:
        df = pd.read_csv(os.path.join(BASE_DATA_PATH, year, year + team + "GAME.csv"), header=None)
